...
class ChatShell extends Component {
    ...
    constructor(props) {
        ...
        Auth.currentAuthenticatedUser({
            ...
            this.ws = new Sockette("wss://amyjijsyk0.execute-api.us-east-1.amazonaws.com/ChatApplicationEndpoint?username=" + this.username + "&email_id=" + this.email + "&chatroom=" + this.chatroom.chatroom, {
                timeout: 5e3,
                maxAttempts: 10,
                onopen: e => {
                    // var j=this.getUsers()
                ...
                onmessage: e => {
                    ...
                    if (("translated_message" in translatejson) === false) {
                        ...
                        console.log("message in json", translatejson["translated_message"])

                        this.setState({ messages: [...this.state.messages, translatejson] })
    ...
    render() {
        return (
            <div id="chat-container" >
                ...
                <ChatForm getMessage={this.getMessage} />
            </div>
        )
    }
}
